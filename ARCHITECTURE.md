...
| Status | Use case |
|---|---|
| 🔺 seeded · fixed | Opening any **unpublished** product's detail view → `NullReferenceException` → HTTP 500 + Error log + error-tagged span — **resolved (AMS incident 55)**: the null review-note dereference in `GetProductByIdQueryHandler` was removed, so unpublished products now load normally |
