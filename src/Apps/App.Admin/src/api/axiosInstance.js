...
// Auth-bypass/demo: derive the API host from the page's own host so the app works
// on both http://localhost:7002 and http://<host-ip>:7002 from a single build
// (a baked localhost URL would resolve to the *client* machine over the LAN).
// The Catalog API is published on host port 7101.
const API_BASE_URL = `${window.location.protocol}//${window.location.hostname}:7101`;
