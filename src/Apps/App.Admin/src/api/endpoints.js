 ...
 *
 * Minimal deployment note: VITE_API_GATEWAY points directly at Catalog.Api
 * (no YARP gateway in this slice), so CATALOG entries below have no
 * "/catalog-service" prefix - Catalog.Api's own routes are mounted at root.
 * The other sections' services (Inventory/Discount/Order/Report/Notification/
 * Communication) were dropped from this deployment; their entries are left
 * unused rather than deleted in case that slice is restored later.
 */
...
export const API_ENDPOINTS = {
  ...
  CATALOG: {
    ...
    GET_ALL_PRODUCTS: "/admin/products/all",
    GET_PRODUCT_DETAIL: (id) => `/admin/products/${id}`,
    CREATE_PRODUCT: "/admin/products",
    UPDATE_PRODUCT : (id) => `/admin/products/${id}`,
    DELETE_PRODUCT: (id) => `/admin/products/${id}`,
    PUBLISH_PRODUCT: (id) => `/admin/products/${id}/publish`,
    UNPUBLISH_PRODUCT: (id) => `/admin/products/${id}/unpublish`,
    GET_CATEGORIES: "/categories",
    GET_CATEGORY_TREE: "/admin/categories/tree",
    GET_CATEGORY_DETAIL: (id) => `/admin/categories/${id}`,
    CREATE_CATEGORY: "/admin/categories",
    UPDATE_CATEGORY: (id) => `/admin/categories/${id}`,
    DELETE_CATEGORY: (id) => `/admin/categories/${id}`,
    GET_BRANDS: "/brands",
    GET_BRAND_DETAIL: (id) => `/admin/brands/${id}`,
    CREATE_BRAND: "/admin/brands",
    UPDATE_BRAND: (id) => `/admin/brands/${id}`,
    DELETE_BRAND: (id) => `/admin/brands/${id}`
  },
