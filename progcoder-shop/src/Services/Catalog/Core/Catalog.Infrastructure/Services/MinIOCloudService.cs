...
public sealed class MinIOCloudService : IMinIOCloudService
{
    ...
    // Browser-reachable host:port for PublicURL, distinct from _endPoint
    // (the container-network address used for the calls above).
    ...
    public MinIOCloudService(IMinioClient minioClient, IConfiguration cfg)
    {
        _minioClient = minioClient;
        _endPoint = _minioClient.Config.Endpoint;
        _publicEndpoint = cfg[$"{MinIoCfg.Section}:{MinIoCfg.PublicEndpoint}"] ?? _endPoint;
        _publicScheme = cfg.GetValue<bool>(cfg[$"{MinIoCfg.Section}:{MinIoCfg.Secure}"]!) ? "https" : "http";
    }
        ...
        try
        {
            ...
            foreach (var f in files)
            {
                ...
                results.Add(new UploadFileResult
                {
                    ...
                    PublicURL = isPublicBucket ? $"{_publicScheme}://{_publicEndpoint}/{bucketName}/{objectName}" : string.Empty,
                });
            }
